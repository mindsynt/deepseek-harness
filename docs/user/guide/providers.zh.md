...
::: tip 表单刻意保持精简
模型页开放让一条路由得以存在的字段——API 密钥、显示名称、API 地址、API 协议——以及每个模型的 ID、显示名称、上下文窗口、最大输出 token 数、图片输入和推理等级。其余所有字段——请求兼容性开关、请求头、超时、重试策略——都在 `$DSH_HOME/settings.yaml` 中设置，也就是模型页写入的同一份文档。可以直接编辑它；浏览器与服务器在同一台机器时，也可以点击设置页顶部的**打开配置文件**打开它。适配器会在下一次请求时重新读取，无需重启任何东西。下面各小节介绍多数网关会用到的字段。
:::
...
因此自定义提供方下的视觉模型需要做一次声明。在模型的**高级**折叠区里，把**图片输入**设为**文本和图片**；存到 `$DSH_HOME/settings.yaml` 中与下面这行等价：
...
对于声明了推理等级的模型，模型选择器会提供**推理等级**菜单。内置提供方的模型从已安装目录继承其等级。手动录入的模型不声明任何等级，因此模型菜单里不会出现推理等级项，由端点自身的默认值决定模型是否思考。请在模型的**高级**折叠区里声明：把**推理等级**设为**自定义等级**，勾选要提供的等级，并为除 `off` 外的每个等级填写其发送值。存到配置文件里与下面的 `reasoningEfforts` 映射等价：
...
- **手动录入的模型没有推理等级菜单**：该模型没有声明任何等级。请在模型页把该模型的**推理等级**设为**自定义等级**，或在 `settings.yaml` 中给它加上 `reasoningEfforts`。
...
- **图片在发送前被拒绝**：该模型未声明图片模态。请在模型页把该模型的**图片输入**设为**文本和图片**，或在 `settings.yaml` 中给它加上 `input: [text, image]`；在 DeepSeek 自身的路由上，请从配置的目录中选择支持图片的条目（默认为 `deepseek-flash`），并确认网关提供该模型且支持图片输入。
