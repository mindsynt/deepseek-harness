...
A vision model on a custom provider therefore needs one declaration. In the model's **Advanced** fold, set **Image input** to **Text and images**; the same value stored in `$DSH_HOME/settings.yaml` is:
...
The model picker offers an **Effort** menu for a model that declares reasoning levels. A built-in provider's models inherit their levels from the installed catalog. A model you enter by hand declares none, so the Effort entry does not appear in the menu and the endpoint's own default decides whether the model thinks. Declare them in the model's **Advanced** fold: set **Reasoning effort** to **Custom levels**, tick the levels to offer, and give each level beyond `off` the wire value to send. The stored profile is a `reasoningEfforts` map, and the same value in `$DSH_HOME/settings.yaml` is:
...
- **The Effort menu does not appear for a model you entered by hand** — It declares no levels. Set the model's **Reasoning effort** to **Custom levels** on the Models page, or add `reasoningEfforts` to the model in `settings.yaml`.
