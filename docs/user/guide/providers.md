...
The model picker offers an **Effort** menu for a model that declares reasoning levels. A built-in provider's models inherit their levels from the installed catalog. A hand-entered model declares none until you check the levels it should offer: in **Settings → Models**, open the model's **Model options** and check them under **Reasoning levels** — each checked level is declared under its own name, and **Off** is declared valueless so dispatch sends nothing for it. A gateway that does not speak OpenAI's `reasoning_effort` usually needs nothing here: the default **Thinking format** inherits the installed catalog's dialect when the provider id or endpoint host matches a shipped provider — `qwen` for a top-level `enable_thinking`, `deepseek` for `thinking: {type}`, and so on — and only falls back to OpenAI detection for a family the catalog does not know. Pick a value in the provider card's customized settings only to override that; the route-level choice applies to every model whose protocol takes it. The same declaration may be written by hand in `$DSH_HOME/profiles/<profile>/cordis.patch.yml`; the YAML form is what a gateway needs when a level's wire spelling is not its own name:
...
- **Only reasoning models fail** — pi-ai sends their system prompt as the `developer` role, which the gateway rejects. Set `compat.supportsDeveloperRole: false` on the route, or pick **system** as the route's **System prompt role** in the Models page.
