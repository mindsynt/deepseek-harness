...
持久密钥是吊销根而非 bearer 生命周期：按[进程绑定的浏览器会话](2026-09-18-process-bound-browser-session.zh.md)的决定，cookie 还会随签发它的进程一起结束，持久密钥只留下绝对的 `cookieMaxAgeDays` 区间。删除记录并重启进程是全局撤销机制；当前 Connection 刻意避免在每个请求上访问凭据提供方。不设置 `Secure` 保留 loopback HTTP，但如果操作者让同一 cookie authority 经未加密网络可达，cookie 会以明文传输。启动 URL 含进程凭据，必须视为敏感输出；运行时诊断不会重复它。
