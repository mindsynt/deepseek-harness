...
The durable secret is a revocation root rather than a bearer lifetime: a cookie also ends with the process that issued it, per the [process-bound browser session](2026-09-18-process-bound-browser-session.md) decision, and only its absolute `cookieMaxAgeDays` interval remained from the durable lifetime. Deleting the record and restarting the process is the global revocation mechanism; the active Connection intentionally avoids credential-provider work on each request. Omitting `Secure` preserves loopback HTTP and permits plaintext transmission if an operator makes the same cookie authority reachable over an unencrypted network. The startup URL contains a process credential and must be treated as sensitive output; runtime diagnostics do not repeat it.
