...
The Client keys persistence on the Host document alone: which page authorities may reach `/api` is the Host fence's decision, as the later [settings persistence decision](../architecture/2026-09-19-settings-persistence-follows-the-host-fence.md) records. Dynamic third-party theme ids remain in-process extensions outside the built-in Host schema; removing one resets the live registry without replacing the last durable built-in preference.
