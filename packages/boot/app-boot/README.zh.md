...
- **进程内模块解析。** launcher 在挂载 profile 条目前，将 runtime resolution 安装到 Node 的 ESM 与 CommonJS 内部 resolver。exports、conditions、子路径、模块缓存和错误码仍由 Node 负责；路由后的 ESM 失败报告原始 importer。显式 CommonJS `paths` 始终保留原生查询，包括指向 profile 内的路径。在源码启动（`node --import tsx/esm apps/cli/src`）下，package exports 指向 `lib/` 的 fallback ESM 条目会解析到根 tsconfig `paths` 为其选定的工作区 `src` 文件，使 tsx 重写的裸导入与 profile 条目对每个工作区包共用同一模块实例；CommonJS 条目仍解析到构建产物 `lib`，因为 `tsx/esm` 不注册 CommonJS hook。
