...
describe('runtime resolution', { concurrent: false }, () => {
  ...
  it('redirects a fallback entry reached through a symlinked package directory', async () => {
    ...
    const real = join(dirname(f.installAnchor), 'packages', 'linked-source-lib')
    ...
    const resolution = await resolutionOf(f)
    expect(resolution.entries.find(entry => entry.name === 'linked-source-lib')?.packageDir).toBe(f.installed)
  ...
  it('leaves a fallback entry that resolves outside its package directory', async () => {
    ...
    file(join(dirname(f.installAnchor), 'node_modules', 'shared', 'index.js'), 'export const probe = "shared"\n')
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    expect(createRequire(join(f.profile.dir, 'entry.cjs')).resolve('escaping-lib'))
      .toBe(join(dirname(f.installAnchor), 'node_modules', 'shared', 'index.js'))
  })
