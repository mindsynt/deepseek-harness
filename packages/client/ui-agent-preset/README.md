...
`agentPresets/list` supplies the roster and the chooser policy; default and visibility changes write the `agent-presets` settings namespace through `settings.update`. The picker, blank-session synchronization, and read-only session label use recorded preset identities. A refused write leaves the effective default and visibility the roster reported standing; connection resets and settings updates refresh the roster.
