...
`agentPresets/list` 提供列表和选择器策略，`agentPresets/read` 为查看器提供一条声明的 YAML；默认值与可见性的修改通过 `settings.update` 写入 `agent-presets` settings 命名空间。选择器、空白会话同步和只读会话标签使用记录的 preset 标识。写入被拒绝时，仍以名单报告的生效默认值与可见性为准；连接重置和设置更新会刷新名单。
