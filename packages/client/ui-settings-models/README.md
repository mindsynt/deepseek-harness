...
The collapsed 自定义设置 fold carries the curated extras: `baseURL` for both families (the deepseek placeholder shows the public endpoint), each adapter's model catalog, and the **display name** and **API protocol** of a pi-ai route the adapter does not ship. For an OpenAI-completions pi-ai route the fold also carries the **Thinking format** select: the route-level `compat.thinkingFormat` pi-ai uses to speak a gateway's reasoning dialect (for example `qwen` for a top-level `enable_thinking`). It is route-scoped because a dialect belongs to the endpoint, and pi-ai skips models whose protocol cannot take it. Leaving it at its default inherits the installed catalog's dialect for a route id or endpoint host the catalog ships; an explicit value overrides that. The fold also carries the **System prompt role** select for the two OpenAI protocols: the route-level `compat.supportsDeveloperRole`, which decides whether a reasoning model's system prompt travels as `developer` or stays `system`. A custom route the add card creates defaults to `system`, because pi-ai infers `developer` for an endpoint it cannot name and most gateways reject that role. Profile `headers` remain deployment configuration in `cordis.patch.yml` or Cordis config and have no Models-page editor. The Provider ID stays fixed: it is the settings key, the name every other namespace and every logged session references, and the stem of a credential reference the page cannot read back to move. Reasoning effort is not a provider-scoped control — it is a per-model capability, and a provider-level value could only be one some models reject. Each model row edits `id`, optional display `name`, optional `contextWindow`/`maxTokens`, input types, and its own reasoning levels; unrelated model fields survive edits. A model also carries per-1,000,000-token **cache-hit**, **cache-miss**, and **output** prices, an optional UTC offset, and ordered half-hour time bands; cache-write tokens bill at the cache-miss price, and rows without pricing are written without a pricing record.
...
The **Reasoning levels** row edits a pi-ai model's `reasoningEfforts`: each checked level is declared under its own name, and **Off** is declared valueless so dispatch sends nothing for it. At least one level other than Off is required; no level checked removes the declaration, leaving a hand-entered model with no levels. Wire-spelling renames stay in `cordis.patch.yml`.
...
The notice step owns its exact copy in `src/client/locales.ts` and its acknowledgement version in `src/onboarding-copy.ts`; on every page the Host's `/api` fence admits, it compares and writes `ui-settings-general.welcomeNoticeVersion` through the shared configuration form, and only an explicit Continue records the current version. The acknowledgement follows the Host fence like every other preference: there is no process-local fallback, and a page the Host refuses sees the shared form's refusal instead of a browser-local acknowledgement. The DeepSeek step targets `deepseek-official` in `llm-deepseek` and renders the existing `ProviderEditor` in credential-only mode inside the shared onboarding modal; `credentials.set` stays the only secret write, and no provider settings are changed.
