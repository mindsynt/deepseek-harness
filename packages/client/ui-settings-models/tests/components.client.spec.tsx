...
describe('ModelsSection', () => {
  ...
  it('validates every adapter-owned model catalog invariant', () => {
    ...
    // The per-model capability fields the editor cards now offer.
    expect(validateDeepSeekModels([{ id: 'model', input: 'text' }]))
      .toEqual({ index: 0, key: 'modelInputInvalid' })
    expect(validateDeepSeekModels([{ id: 'model', input: ['text', 'video'] }]))
      .toEqual({ index: 0, key: 'modelInputInvalid' })
    ...
    expect(validateDeepSeekModels([{ id: 'model', reasoningEfforts: { turbo: 'turbo' } }]))
      .toEqual({ index: 0, key: 'modelEffortsUnknownLevel' })
    expect(validateDeepSeekModels([{ id: 'model', reasoningEfforts: { high: null } }]))
      .toEqual({ index: 0, key: 'modelEffortsLevelValue' })
    expect(validateDeepSeekModels([{ id: 'model', reasoningEfforts: { high: '' } }]))
      .toEqual({ index: 0, key: 'modelEffortsLevelValue' })
    expect(validateDeepSeekModels([{ id: 'model', reasoningEfforts: { off: '' } }]))
      .toEqual({ index: 0, key: 'modelEffortsOffValue' })
    expect(validateDeepSeekModels([{ id: 'model', reasoningEfforts: { off: [] } }]))
      .toEqual({ index: 0, key: 'modelEffortsOffValue' })
    expect(validateDeepSeekModels([{ id: 'model', reasoningEfforts: { off: 'off', high: 'high' } }]))
      .toBeUndefined()
    expect(validateDeepSeekModels([{ id: 'model', reasoningEfforts: { off: null } }]))
      .toEqual({ index: 0, key: 'modelEffortsOnlyOff' })
  })
