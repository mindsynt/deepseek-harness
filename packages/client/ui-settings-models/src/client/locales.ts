...
export const en = {
  ...
  modelPricingHint: 'Per 1,000,000 tokens; use one currency across models. Cache-write tokens are billed at the input (cache miss) price.',
  modelPricingCacheHit: 'Input (cache hit)',
  modelPricingCacheMiss: 'Input (cache miss)',
  modelPricingOutput: 'Output',
  modelPricingBands: 'Time bands (first match wins; an end before the start wraps past midnight)',
  modelPricingUtcOffset: 'UTC offset (minutes)',
  modelPricingUtcOffsetHint: 'Defaults to +480 (Beijing time). Leave blank to use it.',
  modelPricingBand: 'Time band',
  modelPricingBandStart: 'Start',
  modelPricingBandEnd: 'End',
  modelPricingAddBand: 'Add time band',
  modelPricingRemoveBand: 'Remove time band',
  modelInputText: 'Text',
  ...
  reasoningMax: 'Max',
  reasoningNeedsThinkingLevel: 'Keep at least one level other than Off.',
  addModel: 'Add model',
  ...
  modelReasoningInvalid: 'Reasoning levels must offer at least one level other than Off.',
  modelPricingInvalid: 'Each pricing field needs a non-negative decimal, and all three base prices are required.',
  modelPricingTimeInvalid: 'Time bands must start and end on a 30-minute mark, and the two times must differ.',
  modelPricingOffsetInvalid: 'UTC offset must be a whole number of minutes between -720 and 840.',
  ...
  thinkingFormatDefault: 'Default (inferred from the endpoint or catalog)',
  systemPromptRole: 'System prompt role',
  systemPromptRoleSystem: 'system — always send the system prompt as system',
  systemPromptRoleDeveloper: 'developer — send as developer for reasoning models',
  systemPromptRoleDefault: 'Default (inferred from the endpoint or catalog)',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  modelPricing: '价格',
  modelPricingHint: '价格按每 1,000,000 token 计，请所有模型使用同一货币单位；缓存写入 token 按输入（未命中缓存）价格计费。',
  modelPricingCacheHit: '输入（命中缓存）',
  modelPricingCacheMiss: '输入（未命中缓存）',
  modelPricingOutput: '输出',
  modelPricingBands: '分时段价格（按顺序匹配，结束早于开始表示跨午夜）',
  modelPricingUtcOffset: 'UTC 偏移（分钟）',
  modelPricingUtcOffsetHint: '默认为 +480（北京时间），留空即使用默认值。',
  modelPricingBand: '时段',
  modelPricingBandStart: '开始',
  modelPricingBandEnd: '结束',
  modelPricingAddBand: '添加时段',
  modelPricingRemoveBand: '删除时段',
  modelInputText: '文本',
  modelInputImage: '图片',
  ...
  reasoningXhigh: '极高',
  reasoningMax: '最高',
  reasoningNeedsThinkingLevel: '推理等级至少保留一个「关闭」以外的档位。',
  addModel: '添加模型',
  removeModel: '删除模型',
  ...
  modelReasoningInvalid: '推理等级至少保留一个「关闭」以外的档位。',
  modelPricingInvalid: '价格必须是大于等于 0 的数字，且三个基础价格都需填写。',
  modelPricingTimeInvalid: '时段的开始和结束时间需为 30 分钟的整数倍，且两者不能相同。',
  modelPricingOffsetInvalid: 'UTC 偏移需为 -720 到 840 之间的整数分钟。',
  ...
  protocolAnthropicMessages: 'Anthropic Messages',
  thinkingFormat: '思考方言',
  thinkingFormatDefault: '默认（按端点/目录自动推断）',
  systemPromptRole: '系统提示词角色',
  systemPromptRoleSystem: 'system — 始终以 system 角色发送系统提示词',
  systemPromptRoleDeveloper: 'developer — 推理模型以 developer 角色发送',
  systemPromptRoleDefault: '默认（按端点/目录自动推断）',
