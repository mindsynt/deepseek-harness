...
The service provides itself immediately with the schema defaults, then loads the `ui-theme` namespace and writes each accepted theme or font-size change through the Host settings API. Pushed settings changes and reconnects refetch the namespace. The persistence boundary is owned by the [settings persistence decision](../../../.agents/notes/implemented/architecture/2026-09-19-settings-persistence-follows-the-host-fence.md).
