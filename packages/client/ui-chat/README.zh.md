...
当提供方模型设置中带有按路由价格时，每个已完成轮次的用量按钮会带上该轮估算费用，详细模式下输入框统计行末尾会显示可点击的“模型花费”（当前模型在全部会话中的累计费用），默认显示当前月份并将金额保留两位小数。点击后会打开类似 Token 用量的明细弹窗，列出未命中输入、命中缓存输入、输出各自的 token 数与估算金额，并提供“月份”（全部/各自然月）与“范围”（全部会话/仅主会话/仅当前会话）选择器。跨会话读数会汇总每个已列出会话的 `usageByRoute` 与 `usageByDay` 投影，缺失的历史会话会按需加载一次，汇总进行中直接按已汇总到的会话读数计价；已计费但没有配置价格的模型显示“未配置价格”；失败或重试时未披露路由的 attempt 按收尾轮次的路由计价；价格快照就绪前不显示花费读数。价格在首次出现用量界面时按需读取，并在提供方拓扑或连接变化后刷新。
