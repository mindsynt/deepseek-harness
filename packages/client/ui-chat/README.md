...
When the provider model settings carry per-route prices, each completed Turn's usage pill carries its estimated cost, and the trailing end of the composer statistics row reads a clickable **Model spend** for the selected model in detailed mode. It defaults to the current month across all listed Sessions and rounds amounts to two decimal places. Clicking it opens a Token-usage-style breakdown of cache-miss, cache-hit, and output tokens with their estimates, plus **Month** (`All time` / each calendar month) and **Scope** (`All sessions` / `Main sessions only` / `This session only`) selectors. The cross-Session reading joins every listed Session's `usageByRoute` and `usageByDay` projections, loading missing histories once and showing a summarizing state until the join settles. A billed route with no configured price reads `Price not set`; a failed or retried attempt that disclosed no route is priced through the route that closed the Turn, while no cost reading appears before the pricing snapshot is ready. Pricing is read lazily on the first usage-bearing surface and refreshed after provider topology or connection changes.
