...
- **没有 logout 操作**：清除浏览器 cookie 会结束单个浏览器会话，而重启 `dsh` 本来就会结束上一个进程授权过的全部会话；删除 owner 凭据记录则使该撤销成为全局的。
