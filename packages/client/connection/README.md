...
- **There is no logout operation** — clearing the browser cookie ends one browser session, and restarting `dsh` already ends every session the previous process authorized; deleting the owner credential record makes that revocation global.
