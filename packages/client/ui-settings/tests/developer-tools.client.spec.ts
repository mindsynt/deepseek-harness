...
describe('developer tools settings', () => {
  ...
  it('shares one Host-backed preference across consumers and disposes it with the plugin', async () => {
    const ctx = new Context()
    onTestFinished(() => ctx.fiber.dispose())
    const view = {
      ns: DEVELOPER_TOOLS_NAMESPACE,
    ...
    const describeCall = vi.fn().mockResolvedValue({ ok: true, value: {
      writable: true, hasDocument: true, namespaces: [view],
    } })
    const mutate = vi.fn().mockResolvedValue({ ok: true, value: { ...view, value: { enabled: false }, revision: 2 } })
    new TestRemote(ctx, { settings: { describe: describeCall, mutate } })
    ...
    expect(mutate).toHaveBeenCalledWith(
      DEVELOPER_TOOLS_NAMESPACE,
      [{ op: 'set', path: ['enabled'], value: false }],
      1,
    )
    expect(fiber.ctx.configForms.developerTools.enabled.getSnapshot()).toBe(false)
    await fiber.dispose()
