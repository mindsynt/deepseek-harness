...
- **Settings persistence follows the Host, not the page authority** — every page the Host's `/api` fence admits reads and writes the same durable document through one authenticated session; this Client offers no separate process-local mode and no per-page opt-out.
