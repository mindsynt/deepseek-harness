...
describe('workspace browser rows', () => {
  ...
  it('renders an active Workspace and keeps its create action separate from toggling', () => {
    ...
    const group: GroupNode = {
      ...
      branch: undefined, sessionCount: 1, expanded: true, containsCurrent: true, sessions: [],
    }
  ...
  it('labels a Workspace row with its checked-out branch and omits it without one', () => {
    const group: GroupNode = {
      key: 'project', workspaceId: wid('project'), cwd: '/projects/project', createdAt: 0, label: 'Project',
      branch: 'feature/nested-name', sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
    }
    render(<ProjectRowItem group={group} onToggle={vi.fn()} onCreate={vi.fn()} t={t} />)
    expect(screen.getByText('（feature/nested-name）')).toBeTruthy()
  ...
  it('workspace row menu opens on the ellipsis, renames, and shows the danger delete row', () => {
    ...
    const group: GroupNode = {
      key: 'project', workspaceId: wid('project'), cwd: '/projects/project', createdAt: 0, label: 'Project',
      branch: undefined, sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
    }
