...
使用 `dsh-fs-local` 可在宿主文件系统上读取、列出、创建目录、监听、原子写入和编辑文件。相对路径从可配置的基准目录解析，而绝对路径和父目录遍历不受限制。到达同一文件的路径和符号链接共享一个身份。写入保留文件权限，可选版本防护会拒绝陈旧覆盖。直接访问宿主文件时选择本包；需要约束变更时使用 `fs-sandbox`。
...
完整或流式读取任意普通 UTF-8 文本文件，按你选择的上限或按字节窗口读取原始字节，并按稳定名称顺序列出一层目录。递归、幂等地创建目录并一并创建所有缺失的父目录，且不接受 mode 或防护参数：目标已是目录时返回 `created: false`，目标不是目录时报 `FS_NOT_DIRECTORY`。原子地创建或替换文件，并原子地应用字面量文本编辑；两个文件变更操作都按文件串行化，并发写入方绝不会交错。它们的版本防护是可选的：省略它即无条件创建或覆盖，提供它则在文件自上次观察以来发生变化时失败。
...
- **单一文件变更临界区。** 每目标 FIFO 锁串行化读取→防护→写入窗口，并发写入与编辑因此被确定性排序——一方胜出，其余操作看到新版本后因版本陈旧而被拒绝。目录创建是幂等的，无需锁或防护。
- **目录创建先探测。** `mkdir` 先探测目标：已存在的目录报告为未创建，目标不是目录时在任何系统调用前以 `FS_NOT_DIRECTORY` 拒绝，创建失败则映射为 `FS_PERMISSION_DENIED` 或 `FS_IO_ERROR`。
...
| [`src/fsio.ts`](src/fsio.ts) | 不依赖 Cordis 的原始 I/O：探测、读取、目录创建、原子写入、字面量编辑、行尾处理 |
