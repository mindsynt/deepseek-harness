...
The request scope also carries the identity of the host whose execution world interprets the workspace root. An omitted identity, or the built-in local one, reads and observes through the Harness host's own filesystem exactly as before; any other identity addresses that host's open execution world, and an identity naming no open world fails with `workspace-file/host-unavailable` instead of falling back to the Harness host. The Session lookup leaves the identity unset until the Session header carries one, so every read stays on the Harness host today.
