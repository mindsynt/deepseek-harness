...
`workspace/branches` 按需从 `.git/HEAD` 读取每个已注册 Workspace 的检出 git 分支；每个检出还有一个 watcher，在落定写入替换 `HEAD` 时推送 `workspace/branch-changed`。分支是没有任何 Workspace 变更会宣告的外部检出状态，因此不进入持久投影：不是检出的路径、或元数据读不到的路径报告"无分支"，而不是报错。
