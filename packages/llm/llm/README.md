...
- **Declare model pricing** — `LlmModelPricing` describes per-1,000,000-token `inputCacheHit`, `inputCacheMiss`, and `output` rates plus optional `utcOffsetMinutes` and first-match `timeBands`; adapters validate model-entry `pricing` with the exported `LlmModelPricingSchema`, and `dsh-token-meter` consumes those declared rates for cost estimates.
