...
- **声明模型定价**——`LlmModelPricing` 描述每 1,000,000 token 的 `inputCacheHit`、`inputCacheMiss` 与 `output` 费率，以及可选的 `utcOffsetMinutes` 与首个匹配的 `timeBands`；适配器用导出的 `LlmModelPricingSchema` 校验模型条目上的 `pricing`，`dsh-token-meter` 使用这些声明的费率进行成本估算。
