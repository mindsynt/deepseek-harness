...
- **`anthropic-messages` 路由默认不固定缓存副本** —— `compat.sendSessionAffinityHeaders` 除非显式设置否则为 `false`，pi-ai 因此不发送 `x-session-affinity` 请求头，副本由网关负载均衡器选择。在按副本持有 KV cache 的网关上，未见过该前缀的副本会重新计费整段前缀：某条此类路由实测命中率 77%，结构性下限仅 3.6%。在此类路由上设置该开关以发送会话 id。
