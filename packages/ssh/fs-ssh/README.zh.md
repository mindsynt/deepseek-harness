...
`dsh-fs-ssh` 在 SSH 辅助进程的文件系统中提供 `ctx.fs`。文件工具读写、在其中创建目录的文件与远端 Bash、终端、语言服务器和 Node 程序看到的文件一致。远端路径规范化、版本保护及原子修改使用辅助程序旁安装的本地文件系统实现。
...
读取保留共享文件系统错误码。写入、编辑与目录创建将已解析的逐次调用策略发送给辅助程序，由其规范化工作区并在修改所在位置执行策略；创建会返回本次是否真正创建了目录。传输丢失报告 I/O 失败；修改可能已经提交，不会自动重试。
