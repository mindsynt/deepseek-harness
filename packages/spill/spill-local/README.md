...
`dsh-spill-local` saves a caller's oversized text to a private, session-scoped file through the `ctx.fs` filesystem seam and returns that backend's resolved path as the locator, with retrieval guidance to read or grep it. Mount it wherever the composition's execution world has a filesystem backend; spilled text follows that world. Names are unpredictable, each session's files group under a stable directory, and the write is an exclusive create, so a shared root cannot leak output or be redirected by a planted symlink. Configuration selects the root and the startup-cleanup retention period; previews and spill decisions live in other packages.
...
Mount this backend in a composition that spills text through a `ctx.fs` filesystem backend. It registers as the `ctx.spillStore` service that the `dsh-spill-policy` plugin and other callers use.
...
A real storage failure — permissions, no space left, an unwritable root, or no `ctx.fs` backend — rejects the `saveText` call; the caller decides how to degrade. The shipped policy treats the rejection as best-effort and keeps the original inline result.
...
The backend owns storage details only, on one principle: **a spilled artifact must be private and unredirectable**. The root is private (0700 by default), the session directory is a stable hash, the leaf name is unpredictable, and the write is an exclusive create. The storage mechanics live in a Cordis-free module so they are unit-testable without a context.
...
| [`src/store.ts`](src/store.ts) | Cordis-free storage mechanics: private root, session directory, safe-name encoding, create-if-absent write through `ctx.fs` |
| — | No runtime invariant companion is published; this package exposes no independent event sequence or mutable data relation beyond contracts enforced at its owning seam. |

### File naming and write

`suggestedName` is untrusted input, so `encodeSegment` escapes every character outside `[A-Za-z0-9._-]` (and `~` itself) into a `~XXXX` form, making the mapping injective over all JS strings: separators, `../`, NUL, and absolute paths can never escape one segment, and the whole-segment tokens `.`/`..` are escaped too. The write is `ctx.fs.writeText(target, text, { kind: 'createIfAbsent' })` — an existing entry, including one a planted symlink resolves to, is rejected rather than overwritten. Two saves of the same suggested name get distinct random prefixes.
...
- **The startup sweep stays host-local** — locators and writes follow `ctx.fs`, but the one-shot cleanup deletes through the harness host filesystem, so a remote execution world keeps its spill files until that world has its own cleanup.
