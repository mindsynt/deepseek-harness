...
`dsh-spill-local` 把调用方的超大文本经 `ctx.fs` 文件系统 seam 保存到私有的会话级文件，并以该后端解析出的路径作为定位信息返回，同时给出读取或搜索它的取回指引。只要组合中 agent（智能体）的执行世界已有文件系统后端即可挂载它；spill 文本随之落在该世界中。产物名称不可预测、每个会话的文件归入稳定目录，且写入采用排他创建，因此共享根目录既不会泄露输出，也不会被预置的符号链接重定向。配置选择根目录与启动清理保留期；预览与 spill 决策由其他包负责。
...
在通过 `ctx.fs` 文件系统后端 spill 文本的组合中挂载此后端。它注册为 `dsh-spill-policy` 插件与其他调用方使用的 `ctx.spillStore` 服务。
...
每次 `saveText` 调用都会把完整文本经 `ctx.fs` 写入一个新文件，并返回三个字段：`locator`（后端解析出的路径）、`bytes`（精确的 UTF-8 字节数）与 `retrievalHint`——"Use read with offset/limit, or grep this path to search within it."。消费方把该提示展示给模型，模型随后可以用其常规文件工具读取或搜索该文件。

### 文件存放位置

文件存放在 `<root>/session-<hash>/<random>-<safeName>`：`session-<hash>` 是所属会话 id 的短哈希（让同一会话的文件归在一起），`<random>-<safeName>` 把不可预测的十六进制前缀与清理为单个安全路径段的调用方建议名配对。相对 `root` 先在进程工作目录下解析，随后目标路径交给 `ctx.fs.resolve`。
...
真实存储故障——权限不足、磁盘已满、根目录不可写，或缺少 `ctx.fs` 后端——会让 `saveText` 调用以拒绝结束；由调用方决定如何降级。随附策略把拒绝当作尽力而为处理并保留原始内联结果。
...
后端只负责存储细节，建立在一个原则之上：**spill 产物必须私有且不可重定向**。根目录默认私有（0700）、会话目录是稳定哈希、文件名不可预测、写入采用排他创建。存储机制放在与 Cordis 无关的模块中，以便无需上下文即可单元测试。
...
| [`src/store.ts`](src/store.ts) | 与 Cordis 无关的存储机制：私有根目录、会话目录、安全名称编码、经 `ctx.fs` 的 create-if-absent 写入 |
| — | 不发布运行时不变式伴生入口；除由所属 seam 强制执行的约定外，本包不暴露独立事件序列或可变数据关系。 |

### 文件命名与写入

`suggestedName` 是不可信输入，因此 `encodeSegment` 会把 `[A-Za-z0-9._-]` 之外的每个字符以及 `~` 本身转义成 `~XXXX` 形式，使映射对所有 JS 字符串都是单射：分隔符、`../`、NUL 与绝对路径永远无法逃出单个路径段，整段 token `.`/`..` 也会被转义。写入采用 `ctx.fs.writeText(target, text, { kind: 'createIfAbsent' })`——任何已存在的条目（包括被预置符号链接解析到的目标）都会被拒绝而非覆盖。对同一建议名的两次保存会得到不同的随机前缀。
...
- **启动扫描仍在本机执行**——定位信息与写入都跟随 `ctx.fs`，但一次性清理通过 harness 宿主文件系统删除，因此远程执行世界的 spill 文件要等该世界自己的清理。
