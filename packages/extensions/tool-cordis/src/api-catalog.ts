...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async remoteHostOf(sessionId: SessionId): Promise<string | undefined>',
        description: 'Resolve the remote execution world that owns one Session\'s paths, for the in-process Host routes that hand paths to the serving desktop. The answer comes from ApiSessionAgentController.resolvedHostOf — the sidecar, then the Workspace accounting, then the local fallback — so this method adds no second host vocabulary, and a Session on this Harness host is reported as `undefined` rather than as another host the caller must know.',
        parameters: [{ name: 'sessionId', description: 'Session whose execution world is resolved.' }],
        returns: 'the remote host identity interpreting the Session cwd, or `undefined` when that world is the Harness host\'s own.',
      },
      {
